...
import { RACE_REVIVE } from './race.config';
...
describe('RaceService 看广告增值', () => {
  ...
  beforeEach(() => {
    ...
    economy = {
      ...
      getWallet: jest.fn(async () => ({ gameCoin: 200, marketingPoint: 0 })),
    };
    adToken = { consume: jest.fn(async () => undefined) };

    svc = new RaceService(races, pet, economy, {} as any, adToken);
  });
  ...
  describe('doubleReward 奖励翻倍', () => {
    it('已结算且未翻倍：核销凭证并再发一份等额奖励', async () => {
      ...
      expect(adToken.consume).toHaveBeenCalledWith('u1', 'nonce1', 'race_double');
    ...
    it('凭证无效：不发币', async () => {
      races.findOne.mockResolvedValue(settledRace());
      adToken.consume.mockRejectedValue(new BadRequestException('广告凭证无效或已过期'));
  ...
  describe('revive 复活重跑', () => {
    const pending = () =>
      settledRace({ status: 'pending', settledAt: null, rank: 4, rewardCoin: 8 });
    ...
    it('未结算且未用过复活：核销凭证并重掷名次', async () => {
      ...
      expect(adToken.consume).toHaveBeenCalledWith('u1', 'nonce2', 'race_revive');
