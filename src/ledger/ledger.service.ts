...
export class LedgerService {
  ...
  /** 玩家全部资产余额（assetCode -> 可用/冻结）。见 `LedgerQueryService`。 */
  balances(userId: string): Promise<Record<string, BalanceView>> {
    return this.query.balances(userId);
  }

  /** 玩家流水分页（倒序）。见 `LedgerQueryService`。 */
  history(
    userId: string,
    opts: { page: number; pageSize: number; assetCode?: string },
  ): Promise<{ list: EntryView[]; total: number }> {
    return this.query.history(userId, opts);
  }

  /** 后台全局流水分页（倒序）。见 `LedgerQueryService`。 */
  globalHistory(opts: {
    page: number;
    pageSize: number;
    userId?: string;
    ...
    return this.query.globalHistory(opts);
  }
