...
const HOME_TYPES: ItemType[] = ['furniture'];
...
export class HomeService {
  ...
  async getHome(userId: string): Promise<HomeView> {
    ...
    for (const l of layouts) {
      placedCount.set(l.assetCode, (placedCount.get(l.assetCode) ?? 0) + 1);
    }

    const byCode = new Map(defs.map((d) => [d.code, d]));
    // 摆放里可能包含已下架家具（目录只返回上架项），补查一次
    const missing = [
      ...new Set(layouts.map((l) => l.assetCode).filter((c) => !byCode.has(c))),
    ];
    if (missing.length > 0) {
      for (const [code, def] of await this.catalog.getManyByCode(missing)) {
        byCode.set(code, def);
      }
    }
  ...
  async place(
    ...
    return this.lock.withLock(`pet:${userId}`, async () => {
      const owned = await this.items.ownedMap(userId);
      const ownedQty = owned.get(def.code) ?? 0;
      const existing = await this.layouts.find({ where: { userId } });
      const placedQty = existing.filter((l) => l.assetCode === def.code).length;
  ...
  private async occupiedRects(layouts: HomeLayout[]): Promise<Rect[]> {
    ...
    const codes = [...new Set(layouts.map((l) => l.assetCode))];
    const defs = await this.catalog.getManyByCode(codes);
    return layouts.map((l) => {
      const d = defs.get(l.assetCode);
      return {
        x: l.posX,
        y: l.posY,
        w: d?.gridW ?? 1,
