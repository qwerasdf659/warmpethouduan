 ...
 *
 * 为什么放在共享的 trading 层而不是让 items/home 各自暴露方法：清理的**触发时机**
 * 属于流转语义（挂单、赠送、回收、易货成交之后），而 items/home 不该知道交易的存在。
 * 反过来让本层去 import ItemsModule + HomeModule 则会把宠物域整条依赖链拖进来
 * （ItemsModule → PetModule），代价远大于在这里直接操作两张表。
 */
@Injectable()
